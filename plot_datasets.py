...
from sparse_grad_accelerate import replace_bert_layers
...
def load_and_cache_examples(tokenizer, dataset_name, split, block_size=512):
    dataset = load_dataset('glue', dataset_name, split=split)
    dataset = dataset.map(lambda examples: tokenizer(examples['sentence1'], examples['sentence2'], truncation=True, padding='max_length', max_length=block_size), batched=True)
...
def main():
    ...
    train_dataset = load_and_cache_examples(tokenizer, args.dataset_name, args.train_split)
    valid_dataset = load_and_cache_examples(tokenizer, args.dataset_name, args.eval_split)

    avg_train_len, min_train_len, max_train_len = compute_average_length(train_dataset)
    avg_valid_len, min_valid_len, max_valid_len = compute_average_length(valid_dataset)

    logger.info(f"Average training sample length: {avg_train_len}, Min length: {min_train_len}, Max length: {max_train_len}")
    logger.info(f"Average validation sample length: {avg_valid_len}, Min length: {min_valid_len}, Max length: {max_valid_len}")
    ...
    avg_intermediate_sparsity = np.mean(intermediate_sparsity)
    min_intermediate_sparsity = np.min(intermediate_sparsity)
    max_intermediate_sparsity = np.max(intermediate_sparsity)

    avg_output_sparsity = np.mean(output_sparsity)
    min_output_sparsity = np.min(output_sparsity)
    max_output_sparsity = np.max(output_sparsity)

    logger.info(f"Intermediate Layer Sparsity: Average: {avg_intermediate_sparsity}, Min: {min_intermediate_sparsity}, Max: {max_intermediate_sparsity}")
    logger.info(f"Output Layer Sparsity: Average: {avg_output_sparsity}, Min: {min_output_sparsity}, Max: {max_output_sparsity}")
